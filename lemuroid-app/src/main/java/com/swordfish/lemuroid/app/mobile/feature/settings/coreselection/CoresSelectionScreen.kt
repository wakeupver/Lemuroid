...
fun CoresSelectionScreen(
    ...
    // Group systems by the number of available cores: multi-core first, then single-core
    val (multiCore, singleCore) = cores.partition { it.system.systemCoreConfigs.size > 1 }

    LemuroidSettingsPage(modifier = modifier.fillMaxWidth()) {
        if (multiCore.isNotEmpty()) {
            LemuroidCardSettingsGroup(
                title = { Text(text = stringResource(R.string.settings_core_selection_multi)) },
            ) {
                multiCore.forEach { (system, core) ->
        ...
        if (singleCore.isNotEmpty()) {
            LemuroidCardSettingsGroup(
                title = { Text(text = stringResource(R.string.settings_core_selection_single)) },
            ) {
                singleCore.forEach { (system, core) ->
                    val state = rememberMemoryIntSettingState(system.systemCoreConfigs.indexOf(core))
                    LemuroidSettingsList(
                        state = state,
                        title = { Text(text = stringResource(system.titleResId)) },
